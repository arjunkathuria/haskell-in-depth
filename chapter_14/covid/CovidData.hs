...
instance TextShow AccumulatedStat where
  showb (AccumulatedStat pop tc td) = showb pop <> "/" <> showb tc <> "/" <> showb td
...
considerCountry statMapByContinent cd =
  M.insertWith (<>) (cd ^. continent) (fromCountryData cd) statMapByContinent
