...
countryCodeWithRest = CountryCodeWithRest
                        <$> countryCode
                        <*> A.takeWhile notEOL
                        <* endOfLine
...
fullCountryData icode =
  CountryData <$> pure icode
              <*> textField -- continent
              <*> textField -- name
...
statInfo = CountryStat
           <$> populationParser
           <*> populationDensityParser
  where
    populationParser = intField
    populationDensityParser = (option Nothing $ Just <$> double)
