...
import Database.HDBC (fromSql, toSql, SqlValue, quickQuery, fetchAllRows', execute, prepare, run, commit)
...
fiFromList [fid, ttl, desc, len, rate] = FilmInfo {
    filmId = FilmId $ fromSql fid
  , title = fromSql ttl
  , description = fromSql desc :: Maybe Text
  , filmLength = FilmLength $ fromSql len
  , rating = (fromSql rate :: Maybe Text) >>= toMaybeRating
  }
fiFromList _ = error "unexpected result (fiFromList)"

fetchSingle :: (Monad m, Convertible SqlValue a) => String -> [[SqlValue]] -> m a
...
allFilms conn = map fiFromList <$> quickQuery conn selectQuery [] -- empty arr == no parameters
  where
    selectQuery = "SELECT film_id, title, description, length, rating FROM film"
-- Note: The above code is fragile, any change in the order of these fields would error
-- because of the use of fiFromList, unless it was also modified accordingly

totalFilmsNumber :: Connection -> IO Int64
totalFilmsNumber conn = do
  res <- quickQuery conn "Select count(*) FROM film" [] -- again, no parameters so empty arr
  fetchSingle "total Films Numbers: " res

-- now with parameters
filmsLonger :: Connection -> FilmLength -> IO [FilmInfo]
filmsLonger conn len = map fiFromList <$> quickQuery conn select [toSql len]
  where
  ...
  catMaybes <$> mapM (runSingle stmt) films
    where
      selectCats = "Select category.name FROM film"
                   <> " JOIN film_categories USING (film_id)"
...
-- Note: there is NO type safety here. nor the query is checked for correctness
-- All the checks are postponed till runtime
...
assignUnlessAssigned conn cid fid = do
  ...
  if b 
    then
      ...
      run conn insert [toSql cid, toSql fid]
 where
